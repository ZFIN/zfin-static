...
follow-up commit (`87f33d121a`) pulled the three root files back into the main repo
under `docroot/`, on the grounds that they belong to the *application*, not to
the archival content.

**`images/` has the same problem, in a larger form: 51 of its 83 entries are
...
across repo's `home/ source/ frontend/ conf/` (the app side) and across
